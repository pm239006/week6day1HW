...
# internal imports
from dealership_shop.models import Customer, Product, ProdOrder, Order, db, product_schema, products_schema
...
@api.route('/shop')
@jwt_required() # using access token we created
def get_shop():

    shop = Product.query.all() # list of objects - can't send a list of objects thru api calls

    response = products_schema.dump(shop) # takes list of objects and turns it into a list of dictionaries
    return jsonify(response) # jsonify essentially stringifies the list to send to frontend
...
def get_order(cust_id):
    ...
    # grab all order_ids associated with customer
    # grab all the products on the particular order
    ...
    for order in prodorder:
        ...
        prod_data = product_schema.dump(product) # change from object to a dictionary

        prod_data['quantity'] = order.quantity # coming from prodorder table
...
def create_order(cust_id):
    ...
    # looping thru the customer order list of dictionaries for each product
    for product in customer_order:
        ...
        # add price from prodorder table to increment our total order price
...
def update_order(order_id):
    ...
    # based on if the new quantity is higher or lower we either need to decrement or increment total product quantity and order cost
